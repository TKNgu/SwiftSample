...
func DemoMain() throws {
    let window = try Window(name: "Test", rect: Rect(x: SDL_WINDOWPOS.CENTERED.rawValue, y: SDL_WINDOWPOS.CENTERED.rawValue,
        w: 1920, h: 1080), flag: SDL_WINDOW_FULLSCREEN)
    var running = true
    InputHandler.inputHandler.quit = {
        running = false
    }
    // var demo = LogoSmash(screenTexture: window.screenTexture) 
    var demo = PyramidStack(screenTexture: window.screenTexture) 
    
    var startTime = SDL_GetTicks()
    let frameTime = Uint32(1000 / 25)
    let timeStep = cpFloat(1.0 / 25.0)
    var count: Uint32 = 0
    while running {
        InputHandler.inputHandler.update()
        demo.update(timeStep: timeStep)
        demo.draw()
        let runTime = SDL_GetTicks() - startTime
        startTime = SDL_GetTicks()
        if frameTime > runTime {
        ...
        count += frameTime
        if count > 7000 {
            demo = PyramidStack(screenTexture: window.screenTexture)
            count = 0
        }
    }
}
